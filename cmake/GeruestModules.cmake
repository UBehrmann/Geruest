...
# gmake -j can compile before nested OBJECT dirs exist; pre-create at configure time.
function(_geruest_prep_obj_dirs target)
    get_target_property(_sources ${target} SOURCES)
    if(NOT _sources)
        return()
    endif()
    foreach(_src IN LISTS _sources)
        if(NOT _src MATCHES "\\.(cpp|c|cxx|cc)$")
            continue()
        endif()
        file(RELATIVE_PATH _rel "${GERUEST_ROOT}" "${_src}")
        get_filename_component(_obj_subdir "${_rel}" DIRECTORY)
        if(_obj_subdir)
            file(MAKE_DIRECTORY "${GERUEST_BINARY_DIR}/CMakeFiles/${target}.dir/${_obj_subdir}")
        endif()
    endforeach()
endfunction()
...
_geruest_prep_obj_dirs(GeruestCore)
...
if(GERUEST_ENABLE_OBFUSCATION)
    ...
    _geruest_prep_obj_dirs(GeruestObfuscation)
...
if(GERUEST_ENABLE_ASSETS)
    ...
    _geruest_prep_obj_dirs(GeruestAssets)
...
if(GERUEST_ENABLE_WEBSOCKET)
    ...
    _geruest_prep_obj_dirs(GeruestWebSocket)
...
if(GERUEST_HAS_LIBPQ OR GERUEST_HAS_SQLITE)
    ...
    _geruest_prep_obj_dirs(GeruestDatabase)
...
if(GERUEST_HAS_CURL AND GERUEST_ENABLE_EMAIL)
    ...
    _geruest_prep_obj_dirs(GeruestEmail)
