 ...
 * @param content
 */
void Handler::sendResponse(const std::string &status, const std::string &contentType, const std::string &content) const {

	std::string response = buildHeader(status,
												  contentType,
 ...
 * @param contentPath
 */
void Handler::sendFile(const std::string &contentType, const std::string &contentPath,
...
//			sendToLoggerPages("GET: " + path);
		...
		std::string header = buildHeader("200 OK",
										 contentType,
										 contentBuilder->sizeString()
		);
		...
		std::string response = buildHeader("200 OK",
										   contentType,
...
std::string Handler::buildPath(std::string &pathReceived, const std::string &Extension, HTTPRequest* httpRequest)
{
	...
	std::string language = httpRequest->hasHeader("Accept-Language") ?
